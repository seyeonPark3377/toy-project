<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" isELIgnored="false" import="board.model.ResultContent" %>
...
<html lang="ko">
	<head>
	    ...
	    <link rel="stylesheet" href="/SimpleProject/post/css/board-list.css">
	</head>
	<body>
 	<c:choose>
    ...
    <div class="container">
        ...
        <c:when test="${sessionScope.isLoggedIn == true}">
	        ...
	        <div class="content">
	            <h2>게시글 목록</h2>
	            <form action="/SimpleProject/board/list" method="post" id="search">
	                <div class="query-frm">
	                    <div>검색 기준</div>
	                	...
	                	<c:if test="${resultContent.contents.size()!=0}">
		                    <c:forEach var="i" begin="0" end="${resultContent.contents.size()-1}" step="1">
			                    <tr>
			                        ...
			                        <td>
			                            <a href="/SimpleProject/board/read?contentid=${resultContent.contents[i].contentId}">
	            ...
	            <div class="content-reg">
	                <button>
	                	<a href="/SimpleProject/post/board-write.jsp">
	                		글쓰기
               			</a>
           			</button>
	            </div>
	            ...
	            <div class="page-frm">
	            	<c:choose>
	            		<c:when test="${page-5 < 1}">
                            <div class="prev-next">
		                  	    <a href="/SimpleProject/board/list?page=${1}&querySelect=${querySelect}&qorderSelect=${orderSelect}&queryWord=${queryWord}"><<</a>
		                	</div>
                       </c:when>
                       <c:otherwise>
                       		<div class="prev-next">
		                  	    <a href="/SimpleProject//board/list?page=${page-5}&querySelect=${querySelect}&qorderSelect=${orderSelect}&queryWord=${queryWord}"><<</a>
		                	</div>
                       </c:otherwise>
                   </c:choose>
	            	...
	            	<c:choose>
	            		<c:when test="${page-1 >= 1}">
                            <div class="prev-next">
			                    <a href="/SimpleProject//board/list?page=${page-1}&querySelect=${querySelect}&qorderSelect=${orderSelect}&queryWord=${queryWord}"><</a>
			                </div>
                       </c:when>
                       <c:otherwise>
                       		<div class="prev-next">
	                ...
	                <c:forEach var="i" begin="${resultContent.startPage }" end="${resultContent.endPage}" step="1">
	                	<c:if test="${page==i }">
		                	<div class="current">
								<a href="/SimpleProject/board/list?page=${i}&querySelect=${querySelect}&qorderSelect=${orderSelect}&queryWord=${queryWord}">${i}</a>
			                </div>
		                </c:if>
		                <c:if test="${page!=i }">
		                	<div class="prev-next">
	                ...
	                <c:choose>
                       ...
                       <c:otherwise>
                       		<div class="prev-next">
		                  	    <a href="/SimpleProject/board/list?page=${resultContent.totalPage}&querySelect=${querySelect}&qorderSelect=${orderSelect}&queryWord=${queryWord}">></a>
		                	</div>
                       </c:otherwise>
                   </c:choose>
